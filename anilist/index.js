...
// Helper functions
function formatMediaInfo(media, isAnime = true) {
    ...
    if (isAnime) {
        ...
        if (media.duration)
            info += `Episode Duration: ${media.duration} minutes\n`;
    }
    else {
    ...
    if (media.startDate?.year) {
        ...
        if (media.startDate.month)
            info += `-${media.startDate.month.toString().padStart(2, '0')}`;
        if (media.startDate.day)
            info += `-${media.startDate.day.toString().padStart(2, '0')}`;
        info += '\n';
    }
...
function formatCharacterInfo(character) {
    ...
    if (character.dateOfBirth?.year) {
        ...
        if (character.dateOfBirth.month)
            info += `-${character.dateOfBirth.month.toString().padStart(2, '0')}`;
        if (character.dateOfBirth.day)
            info += `-${character.dateOfBirth.day.toString().padStart(2, '0')}`;
        info += '\n';
    }
 ...
 */
export async function searchAnime(params, env) {
    try {
 ...
 */
export async function searchManga(params, env) {
    try {
        const { search, genre, year, format, status, page = 1, perPage = 10 } = params;
        const query = `
 ...
 */
export async function getAnimeDetails(params, env) {
    try {
        const { id } = params;
        const query = `
			query ($id: Int) {
 ...
 */
export async function getMangaDetails(params, env) {
    try {
        const { id } = params;
        const query = `
			query ($id: Int) {
 ...
 */
export async function getCharacterDetails(params, env) {
    try {
        const { id } = params;
        const query = `
			query ($id: Int) {
 ...
 */
export async function searchCharacters(params, env) {
    try {
		...
		`;
        const response = await executeGraphQLQuery(query, { search, page, perPage });
        if (response.errors) {
            throw new Error(`GraphQL errors: ${response.errors.map(e => e.message).join(', ')}`);
        }
        ...
        characters.forEach((character, index) => {
            ...
            if (character.name?.native)
                text += `Native: ${character.name.native}\n`;
            text += `${description}\n`;
            if (character.siteUrl)
                text += `URL: ${character.siteUrl}\n`;
            text += '\n';
        });
        if (pageInfo?.hasNextPage) {
 ...
 */
export async function getTrendingAnime(params, env) {
    try {
		...
		`;
        const response = await executeGraphQLQuery(query, { page, perPage });
        if (response.errors) {
            throw new Error(`GraphQL errors: ${response.errors.map(e => e.message).join(', ')}`);
        }
        const media = response.data?.Page?.media || [];
 ...
 */
export async function getTrendingManga(params, env) {
    try {
        const { page = 1, perPage = 10 } = params;
        const query = `
			query ($page: Int, $perPage: Int) {
