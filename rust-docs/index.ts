// Types for Rust documentation API responses
interface RustCrate {
	name: string;
	version?: string;
	description?: string;
...
async function fetchCrateInfo(crateName: string): Promise<RustCrate | null> {
	try {
		...
		return {
			...
			dependencies: version?.dependencies?.reduce((acc: Record<string, string>, dep: any) => {
...
async function searchCrates(query: string, categories?: string[], limit: number = 10): Promise<CrateSearchResult[]> {
	try {
...
export const get_crate_info = async (params: {
	crate_name: string;
}, env: Environment) => {
	try {
...
export const search_rust_docs = async (params: {
	query: string;
	categories?: string[];
	limit?: number;
}, env: Environment) => {
