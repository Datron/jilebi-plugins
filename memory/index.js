...
function create_relations(request, env) {
    try {
        ...
        const newRelations = relations.filter(relation => {
            ...
            return !knowledgeGraph.relations.some(existing => existing.from === relation.from &&
...
function delete_entities(request, env) {
    try {
        ...
        // Remove entities
        knowledgeGraph.entities = knowledgeGraph.entities.filter(entity => !entityNames.includes(entity.name));
...
function delete_relations(request, env) {
    try {
        ...
        knowledgeGraph.relations = knowledgeGraph.relations.filter(relation => !relations.some(delRelation => relation.from === delRelation.from &&
...
function search_nodes(request, env) {
    try {
        ...
        const filteredEntities = knowledgeGraph.entities.filter(entity => entity.name.toLowerCase().includes(lowerQuery) ||
        ...
        const filteredRelations = knowledgeGraph.relations.filter(relation => filteredEntityNames.has(relation.from) && filteredEntityNames.has(relation.to));
        const result = {
...
function open_nodes(request, env) {
    try {
        ...
        const filteredEntities = knowledgeGraph.entities.filter(entity => names.includes(entity.name));
        // Get entity names for relation filtering
        const filteredEntityNames = new Set(filteredEntities.map(e => e.name));
...
export { create_entities, create_relations, add_observations, delete_entities, delete_observations, delete_relations, read_graph, search_nodes, open_nodes, };
