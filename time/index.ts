 ...
 */
function get_current_time(request: GetCurrentTimeRequest, env: Environment): MCPResult {
	try {
		...
		const targetTimezone: string = timezone === '{local_tz}' ?
 ...
 */
function convert_time(request: ConvertTimeRequest, env: Environment): MCPResult {
	try {
